defmodule DevSecRelAIWeb.HtmlHelpersTest do
  use DevSecRelAIWeb.ConnCase

  alias DevSecRelAIWeb.HtmlHelpers
  ...
  describe "avatar_url/1" do
    test "appends .png to GitHub profile link" do
      assert "https://github.com/devsecrelai.png" == HtmlHelpers.avatar_url("https://github.com/devsecrelai")
    end
  end
  ...
  describe "current_locale_info/0" do
    test "returns the current locale info" do
      assert %DevSecRelAI.LocaleInfo{code: "es", title: "Spanish", original_name: "Español", flag_icon: "es"} ==
  ...
  describe "supported_locales/0" do
    test "returns a list of all supported locales" do
      ...
      assert Enum.all?(locales, &assert(%DevSecRelAI.LocaleInfo{} = &1))
