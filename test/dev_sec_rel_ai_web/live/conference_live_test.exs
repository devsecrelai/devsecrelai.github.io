defmodule DevSecRelAIWeb.ConferencesLiveTest do
  use DevSecRelAIWeb.ConnCase

  import Phoenix.LiveViewTest

  alias DevSecRelAIWeb.ConferencesLive.TestHelpers

  describe "mount/3" do
    test "can mount live view", %{conn: conn} do
      {:ok, _view, html} = live_isolated(conn, DevSecRelAIWeb.ConferencesLive, session: %{})
      assert html =~ "Filter</p>"
    end

    test "applying no filters returns both rows", %{conn: conn} do
      {:ok, view, _html} = live_isolated(conn, DevSecRelAIWeb.ConferencesLive, session: %{})
