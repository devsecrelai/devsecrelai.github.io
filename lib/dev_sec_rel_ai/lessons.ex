defmodule DevSecRelAI.Lessons do
  @moduledoc """
  Implements NimblePublisher and functions for retrieving lessons
  """

  alias DevSecRelAI.Content.Lesson
  alias DevSecRelAI.LocaleInfo

  @ordering Application.compile_env!(:dev_sec_rel_ai, :lessons)
  @future_lessons Application.compile_env!(:dev_sec_rel_ai, :future_lessons)
  ...
  for locale <- @locales do
    path = Path.join([Application.compile_env(:dev_sec_rel_ai, :lesson_dir), locale, "**/*.md"])

    contents =
      quote do
        use NimblePublisher,
          build: DevSecRelAI.Content.Lesson,
          from: unquote(path),
          as: :lessons,
