defmodule Mix.Tasks.DevSecRelAi.Gen.Sitemap do
  use Mix.Task
  ...
  alias DevSecRelAI.{Lessons, Posts}
  alias DevSecRelAI.LocaleInfo
  alias DevSecRelAIWeb.Router.Helpers
  ...
  defp write_to_priv_file!(file_path, contents) do
    full_file_path = Application.app_dir(:dev_sec_rel_ai, file_path)
  ...
  defp page_links(locale, theme, uri) do
    pages = [
      {:live, DevSecRelAIWeb.ConferencesLive},
      {:page, :index},
      {:page, :podcasts},
      {:page, :why},
      {:page, :get_involved},
  ...
  defp lesson_links(locale, theme, uri) do
    config = Application.get_env(:dev_sec_rel_ai, :lessons)
