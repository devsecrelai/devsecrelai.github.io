...
We're using [Rocket Validator](https://rocketvalidator.com) to check [DevSecRelAI](https://dev-sec-rel-ai.com) for HTML and accessibility issues.
...
The current site for DevSecRelAI has more than 1,000 web pages. Imagine if you had to check each web page manually, both for HTML and accessibility issues. That's more than 2,000 checks needed!
...
RV is a paid service but DevSecRelAI contributors have a free Pro account to work on this site. If you're an DevSecRelAI contributor you just need to [sign up](https://rocketvalidator.com/registration/new) for a free account, and contact [jaime@rocketvalidator.com](mailto:jaime@rocketvalidator.com) to have your account upgraded to Pro for free.

## Create your first report

Once you're logged in at RV, you can [create a new report](https://rocketvalidator.com/s/new). You just need to enter a starting URL (https://dev-sec-rel-ai.com), and click on **Validate**.
...
2. Launch ngrok telling it to expose port 4000 and give it a meaningful URL, like `ngrok http 4000 --subdomain dev-sec-rel-ai-joe`.
3. This will create a temporary public URL https://dev-sec-rel-ai-joe.ngrok.io that you can use to validate your dev site on RV.

Just remember to change `joe` by your name or whatever other string you want, but keep `dev-sec-rel-ai` somewhere on the URL to make it easier working with muting rules.
...
* A string to match URLs. It can be a whole URL if you want to be specific, or just a substring like `dev-sec-rel-ai`, which is recommended as it will match both on the beta, staging and production sites, as well as your ngrok instances if you include that on the name.
...
|--------------------------|-----------------|-------------------------------|
| https://dev-sec-rel-ai.com | Attribute “phx- | Required by Phoenix framework |
| https://dev-sec-rel-ai.com | Attribute “@    | Required by Alpine.js         |
| https://dev-sec-rel-ai.com | Attribute “x    | Required by Alpine.js.        |
...
[Latest Stats and Reports for DevSecRelAI](https://rocketvalidator.com/stats/dev-sec-rel-ai.com)
