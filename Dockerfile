...
ARG DEPLOY_DOMAIN="http://localhost:4000"
...
COPY conferences conferences
COPY images assets/static/images

# install npm dependencies
RUN cd assets && npm ci

# generate rss and sitemap static files
RUN mix dev_sec_rel_ai.gen.rss ${DEPLOY_DOMAIN}
RUN mix dev_sec_rel_ai.gen.sitemap ${DEPLOY_DOMAIN}
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/dev_sec_rel_ai ./
